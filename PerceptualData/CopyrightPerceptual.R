...
mix <- ggplot(data = subset_nonrandom, mapping = aes(x = Condition, y = Accuracy)) + 
  ...
  geom_dotplot(data = subset_fa, 
               ...
               position = position_dodge(width=0.8), binwidth = 3, binaxis = "y", stackdir = "center", dotsize = 0.9) + 
  ...
  #scale_color_discrete(name = "Condition", labels = c("full" = "Full-audio", "melody" = "Melody-only", "lyrics" = "Lyrics-only")) + 
...
subset_JP <- subset(data_long, Case>=37)
...
p_byCase <- ggplot(data = subset_removeIrrelevantCases, mapping = aes(x = Condition, y = Accuracy, color = Condition)) + 
  ...
  geom_dotplot(data = subset_normal, 
               mapping = aes(x = Condition, y = Accuracy, 
                             color = Condition, fill = factor(CourtDecision)), 
               position = position_dodge(width=0.3), 
               binwidth = 3, binaxis = "y", stackdir = "center", dotsize = 1) + 
  ...
  scale_shape_manual(name = "Language", values = c("circle" = 1, "rectangle" = 22, "triangle" = 24), 
                     labels = c("circle" = "English",
                                "rectangle" = "Japanese", 
                                "triangle" = "Mandarin")) + 
